

export const airplane = require("../assets/icons/airplane_icon.png");
export const back = require("../assets/icons/back_icon.png");
export const barMenu = require("../assets/icons/bar_menu_icon.png");
export const bed = require("../assets/icons/bed_icon.png");
export const Live = require("../assets/icons/bookmark_icon.png");
export const bus = require("../assets/icons/bus_icon.png");
export const compass = require("../assets/icons/compass_icon.png");
export const eat = require("../assets/icons/eat_icon.png");
export const event = require("../assets/icons/event_icon.png");
export const home = require("../assets/icons/home_icon.png");
export const menu = require("../assets/icons/menu_icon.png");
export const parking = require("../assets/icons/parking_icon.png");
export const search = require("../assets/icons/search_icon.png");
export const starEmpty = require("../assets/icons/star_empty.png");
export const starFull = require("../assets/icons/star_full.png");
export const starHalf = require("../assets/icons/star_half.png");
export const taxi = require("../assets/icons/taxi_icon.png");
export const train = require("../assets/icons/train_icon.png");
export const user = require("../assets/icons/user_icon.png");
export const villa = require("../assets/icons/villa_icon.png");
export const wind = require("../assets/icons/wind_icon.png");
export const painter = require("../assets/icons/paint_brush.jpg");
export const roofer = require("../assets/icons/roofer.jpg");
export const electrician = require("../assets/icons/plug.jpg");
export const plumber = require("../assets/icons/pipe_wrench.jpg");
export const profile = require("../assets/icons/profile.png");
export const favorite = require("../assets/icons/favorite.png");
export const soccerBall = require("../assets/icons/soccer_ball.png");
export const news = require("../assets/icons/news.png");
export const team = require("../assets/icons/team.png");
export const downArrow = require("../assets/icons/down-arrow.png");
export const waiting = require("../assets/icons/waiting.png");
export const close = require("../assets/icons/close.png");
export const schedule = require("../assets/icons/schedule.png");
export const chat = require("../assets/icons/chat.png");
export const message = require("../assets/icons/message.png");
export const pending = require("../assets/icons/pending.png");
export const info = require("../assets/icons/info.png");
export const hinsurance = require("../assets/icons/h-insurance.png");
export const rxdiscount = require("../assets/icons/discount.png");
export const prescription = require("../assets/icons/prescription.png");
export const vaccine = require("../assets/icons/vaccine.png");
export const hcheckup = require("../assets/icons/h-checkup.png"); 
export const close_icon = require("../assets/icons/close_icon.png"); 


// export default {


//     hinsurance,
//     rxdiscount,
//     prescription,
//     vaccine,
//     hcheckup,

    
//     profile,
//     soccerBall,
//     news,
//     favorite,
//     team,
//     waiting,
//     close,
//     schedule,
//     chat,
//     message,
//     pending,

//     info,

//     downArrow,

//     airplane,
//     back,
//     barMenu,
//     bed,
//     Live,
//     bus,
//     compass,
//     eat,
//     event,
//     home,
//     menu,
//     parking,
//     search,
//     starEmpty,
//     starFull,
//     starHalf,
//     taxi,
//     train,
//     user,
//     villa,
//     wind,
//     painter,
//     roofer,
//     electrician,
//     plumber
// }