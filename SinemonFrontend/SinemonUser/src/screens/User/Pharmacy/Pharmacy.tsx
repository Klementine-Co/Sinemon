...
const Pharmacy_renderItem = ({ route, navigation }: accountProp) => {

    const mydata = useAppSelector((state) => state.user?.mydata);
    const rxdiscounts = mydata?.RxDiscount
    const rxinsurances = mydata?.RxInsurance
    const prescriptions = mydata?.Prescription
