...
const EditRXInsur = ({ rxinsurancess}:editrxinsurProps ) => {
  ...
  const [rxinsurances, setPrescriptions] = useState(rxinsurancess);
  ...
  useEffect(() => {
    ...
    function Item({item}:{item:RxInsurance[0]}) {
      ...
      if(item.id_member != undefined){
        ...
        <View style={{flexDirection:"row"}}>
          ...
          <Text style={{color:COLORS.primary, alignSelf:'flex-start', textAlign:'left', marginTop:5}}>Insurer: {item.insurer}</Text>
          <Text style={{color:COLORS.primary, alignSelf:'flex-start', textAlign:'left', marginTop:5}}>Member ID: {item.id_member}</Text>
          <Text style={{color:COLORS.primary, alignSelf:'flex-start', textAlign:'left', marginTop:5}}>Group no: {item.group_no}</Text>
          </View>

         

          <View style={{height:70,width:'50%', justifyContent:"flex-end",alignSelf:'flex-end'}}>
      ...
      <>
        {
          ...
          (visible === true) ? ( <EditingRXInsurances/>): (<FORMS.RXInsurance cancel={setVisible}/>)
