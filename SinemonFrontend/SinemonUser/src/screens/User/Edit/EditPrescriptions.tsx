...
const EditPrescriptions = ({ prescriptionss}:editprescriptionsprops ) => {
  ...
  const [prescriptions, setPrescriptions] = useState(prescriptionss);
  ...
  useEffect(() => {
    ...
    function Item({item}:{item:Prescription[0]}) {
      ...
      if(item.ndc != undefined){
        ...
        <View style={{flexDirection:"row"}}>
          ...
          <View style={styles.listItem}>
           ...
           <View style={{flexDirection:'column', alignItems:'flex-start', width:'40%'}}>
          ...
          <Text style={{color:COLORS.primary, alignSelf:'flex-start', textAlign:'left', marginTop:5}}>NDC: {item.ndc}</Text>
          <Text style={{color:COLORS.primary, alignSelf:'flex-start', textAlign:'left', marginTop:5}}>Lot no: {item.lot_no}</Text>
          </View>

         

          <View style={{height:70,width:'60%', justifyContent:"flex-end",alignSelf:'flex-end'}}>
            <Text style={{color:COLORS.primary}}>Prescription Date: {FORMS.getDate(item.prescribed_date)}</Text>
            {/* <Text style={{color:COLORS.primary}}>Exp Date: {FORMS.getDate(item.expiration_date)}</Text> */}
  ...
  return(
      ...
      {/* <View style={{flexDirection:"row"}}>
        ...
        <TouchableOpacity  onPress={() => setVisible(false)} style={{}}>
              <Text style={{fontSize:20, textAlign:'left'}}>Add Prescription</Text>
         </TouchableOpacity>
        ...
        data={prescriptions}
        keyExtractor={(item, index)  => item.ndc.toString()}
      ...
      <>
        {
          ...
          (visible === true) ? ( <EditingPrecsriptions/>): (<FORMS.Prescription cancel={setVisible}/>)
