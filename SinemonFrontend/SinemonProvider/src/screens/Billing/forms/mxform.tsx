...
const MxForm = ( { route, navigation }: any ) => {

    const secure = false;
    const [ visible, setVisibility ] = useState( secure );

    const claim = useAppSelector( ( state ) => state.claim );
    const service = claim.serviceitems.filter( x => x.ref === route.params.pos ).length > 0 ? claim.serviceitems.filter( x => x.ref === route.params.pos )[ 0 ] : undefined;
    ...
    const renderDXItem = ( { item }: any ) => (
        <View style={ styles.item }>
            <TouchableOpacity
                onPress={ () => { navigation.navigate( 'CategoryFilter', { type: 'modifiers', pos: item.ref } ); } }
