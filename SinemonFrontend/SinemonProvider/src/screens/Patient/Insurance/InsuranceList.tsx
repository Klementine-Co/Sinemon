...
export const InsuranceList = ( { route, navigation }: searchProp ) => {
    const medinsurances = route.params.medinsurance;
    ...
    function Item ( { item }: UserDataItem | any ) {
        ...
        if ( item.id_member != undefined ) {
            return (
                <>
                    <TouchableOpacity onPress={ () => navigation.navigate( "Insurance", { item: item } ) }>{/* onPress={() => {}} > */ }
                        <View style={ styles.listItem }>


                            <View style={ { flexDirection: 'column', alignItems: 'flex-start', width: '30%' } }>
                                <Text style={ { fontSize: ( 14 / 375 ) * ScreenWidth, color: COLORS.primary, alignSelf: 'center', textAlign: 'left', marginTop: 5 } }>Insurer: { item.insurer }</Text>
                                <Text style={ { fontSize: ( 14 / 375 ) * ScreenWidth, color: COLORS.primary, alignSelf: 'center', textAlign: 'left', marginTop: 5 } }>Member ID: { item.id_member }</Text>
                                <Text style={ { fontSize: ( 14 / 375 ) * ScreenWidth, color: COLORS.primary, alignSelf: 'center', textAlign: 'left', marginTop: 5 } }>Group no: { item.group_no }</Text>
                            </View>
