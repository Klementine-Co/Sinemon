...
import { LabForm, PrescriptionForm } from "./forms";

export function Edit ( { route, navigation }: searchProp ) {
  // const [loading, setLoading] = useState(false);
  const userd = useAppSelector( ( state ) => state.user );

  //console.log( 'member data', route );

  //Used in EDIT MODE
  const initialData = undefined;
  const prescriptions = route.params.member.Prescription;
  const rxinsurances = route.params.member.RxInsurance;
  const drxinsurances = route.params.member.RxDiscount;
  const medinsurances = route.params.member.MedInsurance;
  const labs = route.params.member.labs;
  ...
  // ];
  // const labfields = [
  //     {name: 'prov_firstname', label: 'Prov First Name', required: false, type: TYPES.Text},
  //     {name: 'prov_lastname', label: 'Prov Last Name', required: false, type: TYPES.Text},
  //     {name: 'brief_desc', label: 'Brief Description', required: false, type: TYPES.Text},
  //     {name: 'uploaded', label: 'Date', required: false, type: TYPES.Text},
  //     {name: 'status', label: 'Status', required: false, type: TYPES.Number}, 
  //     {name: 'lab', label: 'Lab', required: false, type: TYPES.Image},
  // ];
  ...
  function update ( ITEM, DATA ) {
    ...
    switch ( ITEM ) {
      case "PROFILE":
        update = { ...userd, mydata: { ...route.params.member, Member: [ DATA ] } };
        //  userd.setMyQueue(update);
        return;
      case "LAB":
        route.params.member.labs.push( DATA );
        update = { ...userd, mydata: { ...route.params.member, labs: route.params.member.labs } };
        // userd.setMyQueue(update);
        return;
      case "MEDS":
        route.params.member.Prescription.push( DATA );
        update = { ...userd, mydata: { ...route.params.member, Prescription: route.params.member.Prescription } };
        // userd.setMyQueue(update);
        return;
      case "MED INSUR":
        route.params.member.MedInsurance.push( DATA );
        update = { ...userd, mydata: { ...route.params.member, MedInsurance: route.params.member.MedInsurance } };
        // userd.setMyQueue(update);
        return;
      case "RXINSUR":
        route.params.member.RxInsurance.push( DATA );
        update = { ...userd, mydata: { ...route.params.member, RxInsurance: route.params.member.RxInsurance } };
        // userd.setMyQueue(update);
        return;
      case "DISCOUNT RXINSUR":
        route.params.member.RxDiscount.push( DATA );
        update = { ...userd, mydata: { ...route.params.member, RxDiscount: route.params.member.RxDiscount } };
        // userd.setMyQueue(update);
        return;
      default:
        break;
  ...
  const Profile = () => {
    ...
    return (
      <View style={ { flex: 1, height: '100%', width: '100%', justifyContent: 'center', alignContent: 'center', marginTop: 40 } }>
        {/* <Form
                    title={"Save"} //this is the button title
                    fields={userfields}
                    initialData={user} //used in edit mode
                    onSubmit={(data:object)=>{onSubmit(data, 'PROFILE')}}
                    loading={loading}
                    showImagePicker={showImagePicker}
                    style={{flex:1, height:'100%', width:'90%', alignSelf:'center',}}/> */}
    ...
    return (
      <View style={ { flex: 1, height: '100%', width: '100%', justifyContent: 'center', alignContent: 'center', marginTop: 40 } }>
        {/* <Form
                    ...
                    onSubmit={(data:object)=>{onSubmit(data, 'LAB')}}
                    // loading={loading}
                    showImagePicker={showImagePicker}
                    style={{flex:1, height:'100%', width:'90%', alignSelf:'center'}}/> */}
    ...
    return (
      <View style={ { flex: 1, height: '100%', width: '100%', justifyContent: 'center', alignContent: 'center', marginTop: 40 } }>
        {/* <Form
                    ...
                    // initialData={initialData} //used in edit mode
                    onSubmit={(data:object)=>{onSubmit(data, 'MEDS')}}
                    loading={loading}
                    // showImagePicker={showImagePicker}
                    style={{flex:1, height:'100%', width:'90%', alignSelf:'center'}}/> */}
        <PrescriptionForm />
