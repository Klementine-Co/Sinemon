...
# TODO add in below conditons to above function and if exists use ram over querying DB

# @sio.on('connection', namespace='/queue')
# def connection(sid, data):
#     print()
#     print()
#     print()
#     print('connected: ', dt.now(), data)
#     print()
#     print()
...
#     # else:
#     #     provider = None
#     # queue = Queue.objects.filter(member=member, prov_id=provider, m_status__in='IWBA')

#     if data['from'] == 'member':
#         # print(members)
#         print(data['member'], " connected to server: ", sid)
#         members.update({data['member']: sid})
#         # print(members)
#     if data['from'] == 'provider':
#         print(data['provider'], " connected to server: ", sid)
#         providers.update({data['provider']: sid})
#         conn = Connection.objects.get_or_create(user = data['provider'])
#         conn.update_conn_id(sid)
# print(environ)

# @sio.on('sendText', namespace='/queue')
# async def message(sid, data):
#     print("message ", data)
#     await sio.emit('notification',  data={
#             'data-from-client': sid},
#              namespace='/queue', room=sid)
...
if __name__ == "__main__":
    web.run(app, host=settings.ALLOWED_HOSTS[0], port=8080)
